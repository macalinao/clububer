...
+--------------------------------------------------------+
| Filename: panels.php
| Author: Nick Jones (Digitanium), modded by WEC
+--------------------------------------------------------+
...
if(!defined("ADMIN_PANEL")){
	...
	// Check if Custom Panels Control is infused ///
	$tableFields = mysql_list_fields($db_name,DB_PANELS);
    $columns = mysql_num_fields($tableFields);
    for ($i = 0; $i < $columns; $i++) {
		$field_array[] = mysql_field_name($tableFields, $i);
	}
	if (!in_array('panel_url_list', $field_array)) {
		echo "<center><strong><span style='color:#ff0000'>Warning: You need to infuse Custom Panels Control to get rid of error notices!</span></strong></center><br />";
	}
