...
if(!iPDP_CAN_VIEW
	...
	fallback(INFUSIONS.'pro_download_panel/error.php?type=access');
}
...
$download = new pdpDownload(isset($_GET['did']) && isNum($_GET['did'])
	? $_GET['did']
	: 0);
...
.($is_enabled
	? '<div><label><input type="checkbox" name="disable_smileys" value="'.$val.'"'
		.($val=='0'
			? ' checked="checked"'
			: ''
		).' /> '.$locale['PDP041'].'</label></div>'
: '');
}


function pdp_cleanup_filename($filename)
{
