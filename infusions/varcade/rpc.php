...
$query=dbquery("SELECT * FROM ".$db_prefix."varcade_rating WHERE id='$id_sent' ");
}
...
$result=dbquery("SELECT count(*) FROM ".$db_prefix."varcade_rating WHERE used_ips LIKE '%".$ip_num."%' AND id='$id_sent' ");
...
$seekresult=dbquery("SELECT id FROM ".$db_prefix."varcade_rating WHERE id=$id_sent "); 
@$idexists = mysql_result($seekresult, 0, 0); 

if($idexists==true) 

{
	dbquery("UPDATE ".$db_prefix."varcade_rating SET total_votes='".$added."', total_value='".$sum."', used_ips='".$insert."' WHERE id='$id_sent'");
} else {
	dbquery("INSERT INTO ".$db_prefix."varcade_rating VALUES ('$id_sent', 0, 0, 0, '')");
	dbquery("UPDATE ".$db_prefix."varcade_rating SET total_votes='".$added."', total_value='".$sum."', used_ips='".$insert."' WHERE id='$id_sent'");
}


$query=dbquery("SELECT total_votes, total_value, used_ips FROM ".$db_prefix."varcade_rating WHERE id='$id_sent' ")or die(" Error: ".mysql_error());
$numbers=mysql_fetch_assoc($query);
$count=$numbers['total_votes'];
$current_rating=$numbers['total_value'];
...
"<li class=\"r5-unit\">5</li>\n".
"</ul>".
"<br /> ".$locale['VOT306']." <strong>".@number_format($sum/$added,1)."</strong> ".$locale['VOT302']." ".$added." ".$tense.". ".
"<br /> ".$locale['VOT307']." </p>";
