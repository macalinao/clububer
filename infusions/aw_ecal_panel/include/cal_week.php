	...
	foreach($y_data as $month => $m_data) {
		foreach($m_data as $day => $d_data) {
			...
			foreach($d_data as $event) {
				$days[$day]['events'][] = '<a href="view_event.php?id='.$event['ev_id'].'">'.$event['ev_title'].'</a>';
			}
		}
	}
}
...
foreach($days as $mday => $day) {
	echo '
	<th class="'.$awec_styles['dayofweek'].'"><a href="day.php?date='.$day['date'].'">'.$locale['EC901'][$w++].' '.$mday.'</a></th>';
}
echo '
</tr>
</thead>
