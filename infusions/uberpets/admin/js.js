...
function Search() {
	...
	if(theQuery !== "") {
		...
		var url = '/infusions/uberpets/admin/itemsearch.php?q=' + theQuery;
		xmlhttp.open('GET', url, true);
		xmlhttp.onreadystatechange = function() {
			if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				document.getElementById('lajv').innerHTML = xmlhttp.responseText + ' ';
			} else {
				document.getElementById('lajv').innerHTML = '<div style="height: 30px;"><em><IMG SRC="/infusions/uberpets/img/loading.gif">Searching ...</em></div>';
			}
		};
		xmlhttp.send(null);  
	}
