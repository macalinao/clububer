...
if($review->id) {
	$catid = $review->data['cat_id'];
}


function prp_count_dl($cats, $cat_id) {
	$count = $cats[$cat_id]['count_reviews'];

	foreach($cats as $new_cat_id => $data) {
		if($data['parentcat']==$cat_id
			&& checkgroup($data['access']))
		{
			$count += prp_count_dl($cats, $new_cat_id);
		}
	}
 ...
 * GUI
 */
if($review->id && $review->status==PRP_PRO_OFF) {
	show_info('<img src="icons/messagebox_info.png" alt="'.$locale['prp_info'].'" style="float:left; padding-right:10px;" /> '.str_replace('%s', $review->id, $locale['PRP023']), 'info', true);
}
...
if(isset($catid)) {
	...
	$res = dbquery("SELECT cat_name, top_cat, cat_id, cat_access,
		cat_sorting, cat_desc, count_reviews
		FROM ".DB_PRP_CATS."
	...
	while($localid) {
		...
		if($localid==$catid) {
			$access_group = (checkgroup($topcat['access'])
				? NULL : $topcat['access']);
			$prp_sorting = "dl_".$topcat['sorting'];
		}
	...
	if(!is_null($access_group)) {
		...
		show_info('<img src="icons/lock.png" alt="'.$locale['prp_locked'].'" /> '.str_replace('%s', getgroupname($access_group),
				$locale['PRP215']));
	}
}
...
if(is_null($access_group) && !$review->id && (!isset($catid) || $catid!=0)) {
	...
	$count = prpCore::get_reviews($get,
		$prp_sorting, $rowstart, $prp->settings['per_page'],
		false, $downs);
