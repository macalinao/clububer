...
if(isset($_POST['save'])) {
	...
	if(isset($id)) {
		$ok = dbquery("UPDATE ".DB_PRP_LICENSES.""
			." SET license_name='$name', license_text='$text'"
			." WHERE license_id='$id'");
	} else {
		$ok = dbquery("INSERT INTO ".DB_PRP_LICENSES.""
	...
	if(!ff_db_count("(*)", DB_PRP_DOWNLOADS, "(license_id='$id')") != 0) {
		$ok = dbquery("DELETE FROM ".DB_PRP_LICENSES.""
			." WHERE license_id='$id'");
	}
	fallback(FUSION_SELF);
}
...
<div align='center'>
".$locale['PRP002'].": <input type='text' value='".$data['license_name']."'"
...
<tr>
	...
	<th class='tbl2' width='150'>".$locale['PRP002']
	...
	if($count) {
		...
		$del_icon = '<a href="'.FUSION_SELF.'?id='.$id.'&amp;del=1"'
			." title='".$locale['prp_delete']."'>"
				.'<img src="../icons/editdelete.png" alt="'.$locale['prp_delete'].'" title="'.$locale['prp_delete'].'" class="noborder"></a>';
	}
